...
class Autoencoder(nn.Module):
    def __init__(self, enc = [[28*28,128],[128,64],[64,12],[64,3]], 
            dec = [[3,12],[12,64],[64,128],[128,28*28]]):
        super(Autoencoder,self).__init__()
        self.len = len(enc)
        self.encoder = nn.ModuleList()
        self.decoder = nn.ModuleList()
        for i in range(len(enc)):
            self.encoder.append(layer(in_features = enc[i][0], 
                out_features = enc[i][1], finish = i == self.len-1))
            self.decoder.append(layer(in_features = dec[i][0], 
                out_features = dec[i][1], finish = i == self.len-1))
    def forward(self,x):
